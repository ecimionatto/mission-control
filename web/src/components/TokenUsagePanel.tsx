...
export function TokenUsagePanel({ result }: Props) {
  ...
  const badge = data ? (
    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{data.filesScanned} files</span>
  ) : undefined;
      ...
      {data && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="token-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
            ...
            <Stat label="Cache Write" value={fmt(data.totalCacheWriteTokens)} color={colors.feedback.muted} />
          </div>
