...
import { fetchPipeline } from './modules/pipeline';
...
const modules = {
  ...
  pipeline: withCache('pipeline', fetchPipeline),
};
...
app.get('/api/all', async (_req, res) => {
  const [agentHealth, prs, workflows, cost, achievements, subagents, tokenUsage, research, aiRatio, fleetTracing, opsHealth, kanban, pipeline] =
    await Promise.all([
      modules.agentHealth(),
      modules.prs(),
      modules.workflows(),
  ...
  res.json({ agentHealth, prs, workflows, cost, achievements, subagents, tokenUsage, research, aiRatio, fleetTracing, opsHealth, kanban, pipeline });
});
...
app.get('/api/pipeline', async (_req, res) => res.json(await modules.pipeline()));
